...
size_t interval;
    ...
    if (max_bin%scale!=0)
        ...
        svg_scale (TEXT_WIDTH,top + TEXT_BASELINE,"|");
        for(int i=1; i<=interval*scale0; i++)
        {
            if (i%scale0==0)
                svg_scale(TEXT_WIDTH+i*BLOCK_WIDTH,top +TEXT_BASELINE,"|");
            else
                svg_scale(TEXT_WIDTH+i*BLOCK_WIDTH,top+ TEXT_BASELINE,"-");
        }
        svg_scale(TEXT_WIDTH,top+ 2*TEXT_BASELINE, to_string(0));
        for(int i=1; i<interval*scale0; i++)
        {
            if ((i%scale0!=0) || ((i % scale0== 0) && ((i > scale0))))
                svg_scale(TEXT_WIDTH+i*BLOCK_WIDTH,top+ 2*TEXT_BASELINE," ");
            else
                svg_scale(TEXT_WIDTH+i*BLOCK_WIDTH,top+ 2*TEXT_BASELINE, to_string(i));
        }
        svg_scale(TEXT_WIDTH+interval * scale0*BLOCK_WIDTH,top+ 2*TEXT_BASELINE, to_string(interval * scale0) );
    }
}
